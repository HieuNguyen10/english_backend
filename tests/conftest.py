...
class JiraTestService:
    def __init__(self, jira_settings):
        ...
        self.auth_string = (jira_settings['users'], jira_settings['password'])
        self.url = jira_settings['url'] + '/rest/atm/1.0'
        self.issue_url = jira_settings['url'] + '/rest/controller/latest/issue'
...
def jira_test_service():
    return JiraTestService({
        ...
        'users': os.getenv('JIRA_USER', '/tests'),
...
def app() -> Generator[FastAPI, Any, None]:
    ...
    """
    BaseCustom.metadata.create_all(engine)  # Create the tables.
    _app = get_application()
    _app.add_middleware(DBSessionMiddleware, db_url=SQLALCHEMY_DATABASE_URL)
    yield _app
    BaseCustom.metadata.drop_all(engine)
